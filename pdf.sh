...
if kill -0 $SERVER_PID 2>/dev/null; then
  ...
  google-chrome --headless=new --no-pdf-header-footer --run-all-compositor-stages-before-draw --timeout=10000 --virtual-time-budget=42000 --print-to-pdf="$FILE" http://localhost:$PORT 1>/dev/null
else
